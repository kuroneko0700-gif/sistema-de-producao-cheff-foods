...
/**
 * Aba Mensal - Fechamento de Indicadores Completo.
...
export function Mensal() {
  ...
  const loadData = async () => {
    setIsLoading(true);
    const savedData = await dataService.getMonthlyData(month);
    setData(savedData || []);
    setIsLoading(false);
  };
  ...
  useEffect(() => {
    ...
    const handleUpdate = (e) => {
      if (!e.detail || (e.detail.type === 'mensal' && e.detail.month === month)) {
        loadData();
      }
    };
  ...
  const handleAddRow = () => {
    ...
    setData(updatedData);
    dataService.saveMonthlyData(month, updatedData);
  };
  ...
  const handleUpdateRow = (id, field, value) => {
    const updatedData = data.map(row => {
      if (row.id === id) {
        ...
        // Se o código do produto mudar, buscar no banco de dados
        ...
        // Cálculos Automáticos baseados na lógica da planilha
        // 1. Total Produzido (kg) = Caixas * Peso Caixa (kg)
        updatedRow.totalProduzido = (Number(updatedRow.caixasEmbaladas) || 0) * (Number(updatedRow.pesoCaixa) || 0);

        // 2. Rendimento Esperado (kg) = Real (bat) * Peso Massa (kg)
        updatedRow.rendimentoEsperado = (Number(updatedRow.realBat) || 0) * (Number(updatedRow.pesoMassa) || 0);

        // 3. Rendimento Real (%) = (Total Produzido / Rendimento Esperado) * 100
        ...
        // 4. Perdas (g/pacote) = Peso Médio - Peso Alvo
        updatedRow.perdasGPacote = (Number(updatedRow.pesoMedioPacotes) || 0) - (Number(updatedRow.pesoAlvoPacote) || 0);

        // 5. Sobrepeso Total (kg)
        // Total de pacotes = Total Produzido / (Peso Alvo / 1000)
        ...
        // 6. Sobrepeso (%) = (Sobrepeso Total / Total Produzido) * 100
  ...
  const handleExportExcel = () => {
    ...
    XLSX.writeFile(wb, `Fechamento_Mensal_${month}.xlsx`);
  };
    ...
    <div className="bg-white rounded-[2rem] shadow-xl border border-gray-100 overflow-hidden flex flex-col min-h-[600px]">
      <div className="p-8 border-b border-gray-100 bg-gray-50/50 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-2xl font-black text-gray-800 tracking-tight uppercase">Fechamento Mensal</h2>
            <p className="text-sm font-bold text-gray-500 uppercase tracking-widest">Indicadores de Rendimento e Sobrepeso</p>
          </div>
        </div>
            ...
            {data.map((row) => (
              <tr key={row.id} className="hover:bg-lime-50/30 transition-colors">
                ...
                <td className="px-2 py-2 border border-gray-100 text-center font-medium">
                  {row.rendimentoEsperado.toFixed(2)}
                </td>
                ...
                <td className="px-2 py-2 border border-gray-100 text-center font-bold text-orange-600">
                  {row.perdasGPacote.toFixed(1)}
                </td>
                <td className="px-2 py-2 border border-gray-100 text-center font-black text-red-600">
                  {row.sobrepesoTotal.toFixed(1)}
                </td>
                <td className="px-2 py-2 border border-gray-100 text-center font-black text-red-600">
                  {row.sobrepesoPercent.toFixed(2)}%
                </td>
